...
/*set and calculate motor values */
 private void INIT_ROBOT_SPECS() {
   ...
   max_velocity_tps = (wheel_max_rpm / 60) * COUNTS_PER_WHEEL_REV;
...
/*set starting coordinates */
 ...
 /**
  * This function is executed when this Op Mode is selected. RUN STUFF HERE
  */
 @Override
...
/*TeleOp Manual Control Function */
 ...
 private void MOVE_TARGET(double target_x, double target_y) {
   ...
   target_angle_rad = Math.atan2(change_y, change_x); /**this looks wrong: why is there a pi/180? */
   ...
   turn_mm = Math.PI * ROBOT_WHEEL_TO_WHEEL_WIDTH_MM * (turn_angle_deg / 360);
   ...
   while (opModeIsActive() && frontLeft.isBusy()) {
     // wait until motors stop running.
     idle();
   }
   self_x = target_x;
   self_y = target_y;
   self_heading_deg = Math.min(Math.max(self_heading_deg, 0), 360);
   // normalises value so that we don't get bearings greater than 360
 }
}
