...
public class Turret {
    ...
    private double kP             = 0.05; // tuned for velocity control (ticks/sec per degree of error)
    ...
    public double updateAuto(double bearingDeg) {
        ...
        double velocity = (Math.abs(error) < angleTolerance)
                ? 0
                : Range.clip(pTerm + dTerm, -MAX_POWER, MAX_POWER) * MAX_TICKS_PER_SECOND;

        // Enforce software limits in auto mode too
        int currentPos = turretMotor.getCurrentPosition();
        if (currentPos >= SOFT_LIMIT_COUNTS  && velocity > 0) velocity = 0;
        if (currentPos <= -SOFT_LIMIT_COUNTS && velocity < 0) velocity = 0;
        ...
        return velocity / MAX_TICKS_PER_SECOND; // return normalised -1..1 for telemetry
    }
